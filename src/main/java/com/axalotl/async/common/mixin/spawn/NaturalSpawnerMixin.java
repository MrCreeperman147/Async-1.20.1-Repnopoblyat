...
public abstract class NaturalSpawnerMixin {
    ...
    @Unique
    private static final Constructor<NaturalSpawner.SpawnState> async$spawnStateConstructor;

    static {
        try {
            async$spawnStateConstructor = NaturalSpawner.SpawnState.class.getDeclaredConstructor(
                    int.class,
                    Object2IntOpenHashMap.class,
                    PotentialCalculator.class,
                    LocalMobCapCalculator.class
            );
            async$spawnStateConstructor.setAccessible(true);
        } catch (NoSuchMethodException e) {
            throw new RuntimeException("Failed to find SpawnState constructor", e);
        }
    }

    @Unique
    private static NaturalSpawner.SpawnState async$newSpawnState(
        ...
        try {
            return async$spawnStateConstructor.newInstance(spawnableChunkCount, mobCounts, potentialCalculator, localMobCapCalculator);
        } catch (Exception e) {
            throw new RuntimeException("Failed to create SpawnState", e);
        }
    }
    ...
    private static NaturalSpawner.SpawnState async$createStateVanilla(
        ...
        return async$newSpawnState(spawnableChunkCount, mobCounts, potentialCalculator, localMobCapCalculator);
    }
